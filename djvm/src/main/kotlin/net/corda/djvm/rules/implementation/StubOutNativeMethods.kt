...
import net.corda.djvm.code.ruleViolationException
...
class StubOutNativeMethods : MemberDefinitionProvider {
    ...
    private fun writeExceptionMethodBody(mv: MethodVisitor): Unit = with(mv) {
        ...
        visitTypeInsn(NEW, ruleViolationException)
        visitInsn(DUP)
        visitLdcInsn("Native method has been deleted")
        visitMethodInsn(INVOKESPECIAL, ruleViolationException, "<init>", "(Ljava/lang/String;)V", false)
        visitInsn(ATHROW)
    }
